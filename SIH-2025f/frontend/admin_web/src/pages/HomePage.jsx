...
const TaskRouting = () => {
    ...
    imageInput.onchange = async (e) => {
      ...
      try {
        const res = await fetch(
          `${API_BASE_URL}/issues/agent-update/${issue_id}`,
          {
            method: "POST",
            headers: { Authorization: `Bearer ${token}` },
            body: formData,
        ...
        showNotification("success", `New status: ${data.aiUpdate.status}`);
        fetchTasks(); // Refresh task list
      } catch (err) {
        showNotification("error", `❌ AI update failed: ${err.message}`);
      } finally {
  ...
  return (
    <div className="bg-gray-50 min-h-screen p-6 relative">
      ...
      {notification && (
        <div
          className={`fixed top-4 left-1/2 -translate-x-1/2 w-60 z-50 px-4 py-3 rounded-lg shadow-md text-sm flex items-center gap-2 
            ${
